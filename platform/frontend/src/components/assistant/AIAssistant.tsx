...
type AssistantStatus = { configured: boolean; model: string; readOnly: boolean; voiceMode: string; tenant: string };
type ChatResponse = { message: string; citations: Citation[]; model: string; conversationId: string; readOnly: boolean };
...
const suggestionsByPath: Array<[RegExp, string[]]> = [
  ...
  [/disputes/, ['What are the strongest dispute candidates?', 'Which evidence gaps block submission?']],
  ...
  return <>
    {open && <section aria-label="AI operations assistant" className="fixed bottom-24 right-4 z-[70] flex h-[min(650px,calc(100vh-7rem))] w-[min(410px,calc(100vw-2rem))] flex-col overflow-hidden rounded-2xl border border-blue-200 bg-white shadow-2xl dark:border-slate-700 dark:bg-slate-900">
      <header className="flex items-center justify-between bg-gradient-to-r from-blue-700 to-indigo-700 px-4 py-3 text-white">
        <div className="flex items-center gap-3"><span className="grid h-9 w-9 place-items-center rounded-full bg-white/15"><Sparkles size={19}/></span><div><h2 className="font-semibold">Operations AI</h2><p className="text-xs text-blue-100">{status?.model || 'Tenant-scoped assistant'} · read only</p></div></div>
      ...
      <div ref={scrollRef} className="flex-1 space-y-4 overflow-y-auto bg-gray-50 p-4 dark:bg-slate-950">
        {!messages.length && <div className="space-y-4"><div className="rounded-xl border border-blue-100 bg-white p-4 text-sm leading-6 text-gray-700 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-200"><strong className="block text-gray-900 dark:text-white">Ask about your operational data.</strong>I can analyze fleet, performance, routes, payroll, disputes, costs, and source health. I cannot submit or modify anything.</div><div className="space-y-2">{suggestions.map((suggestion) => <button key={suggestion} type="button" onClick={() => void send(suggestion)} className="block w-full rounded-xl border border-gray-200 bg-white px-3 py-2.5 text-left text-sm text-blue-700 hover:border-blue-300 hover:bg-blue-50 dark:border-slate-700 dark:bg-slate-900 dark:text-blue-300 dark:hover:bg-slate-800">{suggestion}</button>)}</div></div>}
      ...
      <form onSubmit={submit} className="border-t border-gray-200 bg-white p-3 dark:border-slate-700 dark:bg-slate-900"><div className="flex items-end gap-2"><textarea value={input} onChange={(event) => setInput(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter' && !event.shiftKey) { event.preventDefault(); void send(); } }} rows={2} maxLength={4000} placeholder="Ask about your data…" className="min-h-[46px] flex-1 resize-none rounded-xl border border-gray-300 bg-white px-3 py-2 text-sm text-gray-900 focus:border-blue-500 dark:border-slate-700 dark:bg-slate-950 dark:text-white"/><button type="button" onClick={toggleListening} className={`grid h-11 w-11 shrink-0 place-items-center rounded-xl ${listening ? 'bg-red-100 text-red-700' : 'bg-gray-100 text-gray-600 dark:bg-slate-800 dark:text-slate-300'}`} aria-label={listening ? 'Stop listening' : 'Talk to assistant'}>{listening ? <MicOff size={19}/> : <Mic size={19}/>}</button><button type="submit" disabled={!input.trim() || loading || status?.configured === false} className="grid h-11 w-11 shrink-0 place-items-center rounded-xl bg-blue-600 text-white hover:bg-blue-700 disabled:cursor-not-allowed disabled:opacity-40" aria-label="Send message"><Send size={18}/></button></div><p className="mt-2 text-[10px] text-gray-400">AI can make mistakes. Verify cited operational records before acting.</p></form>
    </section>}
