...
type InvitationResult = { member: Member; invitationSent: boolean; deliveryMode?: string };
...
export const canResendInvitation = (member: Pick<Member, 'status'>) => member.status === 'invited';
...
export const UsersRolesPage: React.FC = () => {
  ...
  const resend = useMutation({
    mutationFn: (member: Member) => api.post<InvitationResult>(`/members/${encodeURIComponent(member.identitySubject)}/resend-invitation`),
    onMutate: () => setNotice(null),
    onSuccess: result => setNotice({ kind: 'success', message: result.invitationSent ? `Invitation email resent to ${result.member.email}.` : `Invitation resend validated for ${result.member.email}; local development does not send email.` }),
    onError: error => setNotice({ kind: 'error', message: apiError(error) }),
  });
