...
const AmazonConnectorSessionPage: React.FC = () => {
  ...
  const [status, setStatus] = useState('Connecting to the private Amazon browser…');
  ...
  const refreshFrame = useCallback(async () => {
    ...
    if (response.status === 410) {
      setStatus('Amazon authentication verified. Returning to Connections…');
    ...
    setStatus('Private tenant browser · Amazon credentials are sent directly to this browser.');
  }, [navigate, sessionId]);
  ...
  return <main ref={sessionRef} className="min-h-screen bg-slate-950 p-4 text-white outline-none" onKeyDown={key} tabIndex={0}>
    <header className="mx-auto mb-3 flex max-w-[1440px] flex-wrap items-center justify-between gap-3">
      <div><h1 className="flex items-center gap-2 text-lg font-semibold"><Lock size={18} />Secure Amazon connection</h1><p className="text-xs text-slate-400">{status}</p></div>
    ...
    <section className="mx-auto max-w-[1440px] overflow-hidden rounded border border-slate-700 bg-black shadow-2xl">
      {frame ? <img ref={imageRef} src={frame} onClick={click} onWheel={(event) => { event.preventDefault(); void send({ type: 'scroll', deltaY: event.deltaY }); }} alt="Private Amazon browser" className="block w-full cursor-crosshair select-none" draggable={false} /> : <div className="grid aspect-[16/10] place-items-center"><Loader2 className="animate-spin text-blue-400" size={30} /></div>}
    </section>
